...
int main() {
    ...
    sha.addShaderByPath("shader/vertex1_light.glsl", VERTEX_SHADER);
    std::cout << "after vertex shader" << std::endl;
    sha.addShaderByPath("shader/fragment1_light.glsl", FRAGMENT_SHADER);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        std::cout << "light model: " << glm::to_string(lightModel) << std::endl;
